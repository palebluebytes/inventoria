...
**Recipe Twin**:
A **named**, reusable recipe **template**: a schema.org/Recipe (ADR-0021) holding a name, an ordered ingredient list of pure references to food Digital Twins with amounts, a yield, and optional description, source url, image, and instructions. It stores no nutrition of its own — per-serving macros derive from the referenced ingredient twins. It only _seeds_ a Recipe Instantiation with defaults; it never governs one. Unlike a food Digital Twin it is composite (built from references to other twins) and is a default, not a nutrition authority. **The name is what makes it one**: the same structure without a `recipe/name` is an **Impromptu Recipe**, and the recipe library is exactly the twins that carry one (ADR-0110).
_Avoid_: Recipe (when the logged occasion is meant), recipe definition, Digital Twin (external-DB sense)

**Impromptu Recipe**:
A `recipe:` twin with **no** `recipe/name` — a dish you assembled on one day rather than a recipe you sat down and wrote (ADR-0110). Structurally it is a **Recipe Twin** in every other respect and its loggings are ordinary **Recipe Instantiations**; what it lacks is a name, and that absence is the whole of the distinction. It is excluded from the recipe library and the log sheet's Recipe tab, and from nothing else: it appears on its day, in reports, in **Past meal** copy, in a sent meal and in search, labelled from its own occasion's frozen ingredient rows rather than from anything stored. Its entity id is derived from its sorted ingredient references, so assembling the same things again lands on the same twin instead of minting a second. Giving one a name promotes it to a **Recipe Twin** and reaches every occasion of it at once. The Recipes screen lists them under the heading _Impromptu recipes_, one row per occasion.
_Avoid_: Unnamed recipe, occasion (ADR-0106 uses that for the sizing of one), one-off, throwaway recipe, draft recipe, dish, grouping
...
**Correction**:
A change to something already logged, written as further datoms **on the event it corrects** and never as a second event (ADR-0111 §1): a new amount, a re-derived `event/metrics`, a re-derived `event/instantiation`, a different **Meal Type**. The occasion keeps its id, its place in its meal and its clock, and every superseded value stays in the ledger behind the winner — so two devices correcting the same occasion converge on the later one and leave one row. What separates it from its neighbours is **arity**: a correction is 1 → 1, a removal 1 → 0, a log or a **Past meal** copy 0 → 1, and consolidating foods into a recipe N → 1. That last is the only act with no single entity to append onto, so it is the only one that still mints an event and writes `event/replaced_by`. A correction reports no id, because the id its caller already holds is the only one there is.
_Avoid_: Edit (that is the screen; the write is a correction), retract-and-replace, re-log, supersession, replacement (a correction produces none)
...
**Occasion fold**:
The ingredients of a logged recipe, open on the day under the row that logged them (ADR-0022, amended 2026-09-17). The row above it is the day's entry and is unchanged — the same card, the same ✕, the same long-press into a **Selection**. Each ingredient under it is drawn as the logged row it behaves like, the day's own name-over-amount row, tapped to open the app's own ingredient picker; what marks them as contents is the rule they hang on and their inset under the parent's name, not a shape of their own. Every act writes at once, because these rows are another view of facts the occasion already records rather than a second editing mode over them. The rows sit flush and share their seams, so a dish's contents read as one list rather than as a stack of cards. What the fold corrects is **what is in** the dish; how much of the dish was eaten is still the correction sheet's question alone, reached by the recipe mark on the parent row rather than by anything inside the fold. It is open or shut for this sitting only, and nothing remembers which.
_Avoid_: Accordion, drawer, expander, inline editor, sub-rows, children (the rows are the occasion's contents, not entries of their own)
...
**Selection bar**:
The strip a **Selection** raises at the foot of the Food screen: a `✕` and five drawn verb marks. The `✕` leads so no status line can wrap it away, and it is the only one on screen, because a selected row swaps its own remove mark for the tick. The bar writes nothing — it carried an `N selected ›` control, which both restated what the ticked rows already say and served as the unmarked door to the Selection's nutrition panel; the hand-off verb now opens that panel, which is where the **Way out** lives. The count survives on the verbs' own labels, so a screen reader still hears the size of what it is about to act on. One status line serves every verb and is silent on success. The bar deliberately covers the tab bar, which is what makes a Selection a mode, and is why `Escape` and the platform back gesture both leave it. Labelled verbs were measured and do not fit at 360px, which is why the marks are drawn rather than written, and the fifth mark is what took the gaps between them one step down the space scale rather than any target below 48px. Two of the five are one act reached two ways: the combine verb consolidates the Selection into an **Impromptu Recipe** where it stands, and the recipe verb opens the builder to do the same with a name and a form in front of it. See ADR-0088 §2 and §3, and its Amendment of 2026-09-17.
