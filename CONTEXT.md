...
**Occasion fold**:
The ingredients of a logged recipe, open on the day under the row that logged them (ADR-0022, amended 2026-09-17). The row above it is the day's entry and is unchanged — the same card, the same ✕, the same long-press into a **Selection**. Each ingredient under it is drawn as the logged row it behaves like, the day's own name-over-amount row, tapped to open the app's own ingredient picker; what marks them as contents is the rule they hang on and their inset under the parent's name, not a shape of their own. Every act writes at once, because these rows are another view of facts the occasion already records rather than a second editing mode over them. What the fold corrects is **what is in** the dish; how much of the dish was eaten is still the correction sheet's question alone. It is open or shut for this sitting only, and nothing remembers which.
