...
**Search index**:
The committed artifact the food search reads, one row per Reference food: identity, the fields ranking reads, the macros a result row renders, the household portions, and the reference to any SR Legacy twin whose values the row borrowed. Generated from USDA's bulk archives with the reference-food filters already applied, so it holds the 4,319 survivors rather than all 7,974 food identities, and the filters run once per generation instead of once per keystroke. Every row carries an energy value, because a record that reports none cannot be logged and does not ship. A row's description is USDA's own, less the parts that do not name the food — a commercial origin and USDA's offal aisle label are removed at generation time, so what ships is the name a person reads and searches. See ADR-0047, ADR-0048 and ADR-0056.
