...
**Distinguishing axis**:
A dimension the USDA records under one head vary along that names something true of a food **when it was bought** — a variety, a part, a fat content, a cut, a dried state. Each of its values is a different Reference food and gets its own row. Drying distinguishes: dried apricots are bought as dried apricots, which is ADR-0104's shopper test and not a claim about whether a fresh form exists — a fact about USDA's coverage decides nothing here. The exception is an Adjudicated head phrase, where a dehydrated form that arrives sealed and labelled rather than scooped out of a bin is dropped instead: milk powder and dried egg, and no prune. See ADR-0103, ADR-0104 and ADR-0061 §3.
_Avoid_: Form, variant axis, real axis

**Residual description**:
What a USDA description reads as once every Collapsing axis segment is struck out of it. It is the key that decides which records are one food, and it is also the name a merged group's Representative ships under: `Beef, flank, steak` for `Beef, flank, steak, separable lean and fat, trimmed to 0" fat, choice`. The strip is licensed by the collapse rather than by the name, so a group of one and a Coverage hole both keep the whole name USDA published, and a residual another row already answers to is not taken. Records are grouped on it with commas, hyphens, slashes and repeated whitespace normalised away, because USDA spells one cut several ways — `Beef, round, top round, steak` and `Beef, round, top round steak` are one food — and a difference of punctuation is not a difference of food. Nothing carrying meaning is normalised. The grouping key normalises; the shipped name does not. See ADR-0103.
_Avoid_: Stripped name, base description, canonical name (every row has a residual; only a merged group's Representative ships under one)

**Collapse group**:
The set of USDA records sharing one Residual description, which is to say the records of one food at every trim, grade and separation USDA published it at. A group of one is the ordinary case, and overwhelmingly so: 2,418 rows fall into 2,037 groups, of which 1,858 hold a single record. What is left to collapse once ADR-0104 has removed the cooked half is purely butchery, so the rule reaches four head phrases — `Beef`, `Lamb`, `Pork` and `Veal` — and moves no other. See ADR-0103.
_Avoid_: Duplicate set, permutation group, cluster

**Representative**:
The one record of a Collapse group whose panel the Search index ships, chosen by the fuller nutrient panel and then the lower `fdcId`. It is always a record USDA published, never an average of the group: an average is a number nobody measured and no `fdc:` entity can carry. A record is eligible to represent a group only where the name the group ships under stays true of it, so a record stating a separation — `separable lean only`, `separable fat` — stands for nothing. Separation is the only axis left carrying such a value: preparation carried the other and left the roster with ADR-0104, which removes the cooked records rather than merging them. Trim and grade refuse nothing: the group exists because those are the same food, and refusing a record for stating one would re-import the distinction the collapse just erased. The spread the group loses is real and is not recorded anywhere on the row — on beef sirloin, 12% of the calories across trim and 6% across grade. See ADR-0103.
_Avoid_: Winner, canonical record, survivor (which is ADR-0051's word for a twin merge), average row

**Coverage hole**:
A Collapse group of more than one record where no record is eligible to be its Representative. It ships anyway: its fullest-panel record under that record's whole, unstripped name, which is the treatment a group of one already gets — `Quinoa, cooked` is the only quinoa USDA publishes and ships saying so. What a hole forbids is the strip, never the row, so it blocks no head and a Curated stand-in improves it rather than gating it. The first instances arrived when the rule shipped, and there are five, every one of them beef: a cut USDA assayed `separable lean only` and no other way, at three grades — t-bone steak, bottom round roast, top sirloin petite roast, ribeye cap steak and ribeye petite roast. Plain skinless chicken breast was named as the confirmed hole and is not one, `fdc:171077` being a plain raw breast at 120 kcal. See ADR-0046 and ADR-0103.
...
**Pairing target**:
One row per food of a USDA record that was **cooked before it was measured** — the records ADR-0104's shopper test keeps out of the Search index, re-admitted as 1,182 collapsed rows for one purpose only. It is reachable solely after a person gives a **Declared state** of _cooked_, is never returned by the food search, and is never directly loggable: the only way to a Pairing target is through a pairing. Named by that route, which is what makes it unconfusable with a **Reference food** — it is a food you could not scoop out of a bin. Its rows carry the same fields as a Search index row under a distinct type, so a mistaken call is a compile error rather than a leak. See ADR-0113 §11.
_Avoid_: Cooked reference food, prepared reference, USDA cooked record, cooked corpus

**Pairing index**:
The artifact the pairing search reads once a person declares a pack cooked, one row per **Pairing target**, mirroring the **Search index**'s shape and ranking. Fetched on demand and precached by neither Facet, so a person who never declares a pack cooked never asks for it. It carries no **Vocabulary map** of its own: the shipped Search index's is always already loaded, and duplicating it would make two copies of a map read as one. See ADR-0113 §11.
_Avoid_: Cooked index, second index, pairing corpus (the two artifacts together are not given a name)

**Pairing nutrient store**:
The artifact holding every nutrient USDA reports for a **Pairing target**, mirroring the **Nutrient store**. Fetched only when a person accepts a row, on ADR-0047's rule that search never reads a nutrient and staging reads all of them, which keeps 1.17 MB of panels off the act of looking. See ADR-0113 §11.
_Avoid_: Cooked nutrient store, pairing panels

**Declared state**:
A person's answer about the pack in their hand — _cooked_, or _as you bought it_ — which decides whether the pairing search offers **Pairing targets** or **Reference foods**, and **is never recorded**. Two values and no third, because an _I don't know_ that showed both sets would re-admit by the option nobody reads carefully the confusion the partition exists to refuse. It defaults to as-bought, so it is a widening act rather than a gate in front of pairing, and nothing is stored because the target's own `fdc:` id already is the state. It grades a food-identity claim about your own jar, never a composition one. See ADR-0113 §11.
_Avoid_: Cooked flag, preparation state, state toggle, form (which named a staging picker that was refused), the cooked question
...
**Vocabulary map**:
The table of phrases the Search index does not use, each mapped to the phrases it does — `aubergine` to `eggplant`, `courgette` to `zucchini`, `minced beef` to `ground beef`. It ships in two sections of the Search index and is read as one map: `vocabulary_off`, derived from Open Food Facts' ingredients taxonomy and carrying its ODbL licence, and `vocabulary_local`, eight everyday names OFF does not carry either — seven British (`gammon`, `caster sugar`) and one spelling (`soymilk`) — written by hand outside that derivative. It is a **retrieval fallback**: a query that already retrieves something is answered exactly as it is today. It is asked what the query strip left rather than what was typed, so `raw aubergine` reaches the key `aubergine` instead of missing every key for holding a word the corpus stopped saying, and a key that would itself be stripped is refused at generation. A food reached through it is displayed under both names — `Eggplant, raw (aubergine)` — so a search that quietly answered with another word says which word. The key is bracketed rather than comma-appended, because a comma made it read as one more of USDA's qualifiers and 211 of the 452 keys that lead anywhere share a word with the name they land on. See ADR-0049.
...
**Curated pairing**:
One hand-authored row asserting that a barcode's substance is a named USDA food — `{ gtin, fdcId, set, product, captured, ground }` — shipped so that a person meeting that pack is offered the row rather than left to search for it. `set` says which artifact the `fdcId` lives in and is not lookup metadata: a row naming a **Pairing target** _is_ the assertion that the pack is cooked. It **pre-selects and never accepts**: the screen shows the USDA row's own description and the **Declared state** the row asserts, one explicit act takes both, and a person's own live pairing always wins over it. A barcode already carrying a **Curated stand-in** may never have one, because a stand-in is admitted on a proof that no reference table carries that food. See ADR-0113 §14.
_Avoid_: pairing list (spent on the **Twin ledger**, and spent with the inverted sense — ADR-0048 §4 forbids a list that _creates_ merges), **Pairing target** (that is the row being pointed at, not the pointing), **Curated stand-in** (the mirror: that one answers a search where no reference food exists; this one points a barcode at one that does), GTIN map, barcode mapping, match

**Curated pairing table**:
The artifact holding every **Curated pairing**: one hand-authored TypeScript module on the **Curated stand-in**'s precedent, type-import-only so a bare runner can load it with no install step. What keeps it curated is a shape and a rate rather than a check — no check can prove a row was hand-authored, and what the commitment really rests on is that every row carries a `ground` a reviewer reads, so a submission path scales only by deleting that field. It has a **ceiling of 100 rows** asserted by a test, where reaching the ceiling means re-arguing whether a shared store is ours to hold rather than raising the number. It is a prior, never a fact. See ADR-0113 §14.
_Avoid_: pairing list, pairing database, shared pairing store (which is the thing this exists instead of), lookup table
...
**Batch weight**:
What a finished batch of a recipe weighs, in grams and only grams (ADR-0106). It has no Panel to read an **Amount Unit** off and exactly one honest source, a scale, so admitting millilitres would mean asking "which?" about a number that can only have come from one place. A **Recipe Twin** remembers one as a default (`recipe/batch_weight`), a **Recipe Instantiation** may override it for that occasion and never writes the override back, and the occasion freezes it beside its rows so a logged meal says 160 g of a 480 g pot forever. It is **not** the sum of the ingredient rows: a pot does not weigh what went into it, since a stew simmers off water and rice absorbs it, so Σ-raw and the finished weight are different quantities rather than approximations of one another. A recipe nobody weighed has none, which is ordinary, and is sized by its serving count instead.
_Avoid_: Yield (that is the count, `recipe/yield`), portion weight (that is the occasion's numerator), total weight, cooked weight, Σ of the ingredients
