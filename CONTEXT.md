...
**Selection bar**:
The strip a **Selection** raises at the foot of the Food screen: a `✕`, the count, and three drawn verb marks. The `✕` leads so no status line can wrap it away, and it is the only one on screen, because a selected row swaps its own remove mark for the tick. The count is a control rather than a label — it opens the Selection's nutrition panel, which is where the **Way out** lives, so the bar carries no share verb. One status line serves every verb and is silent on success. The bar deliberately covers the tab bar, which is what makes a Selection a mode, and is why `Escape` and the platform back gesture both leave it. Labelled verbs were measured and do not fit at 360px, which is why the marks are drawn rather than written. See ADR-0088 §2 and §3.
