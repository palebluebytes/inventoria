...
**Occasion fold**:
The ingredients of a logged recipe, open on the day under the row that logged them (ADR-0022, amended 2026-09-17). The row above it is the day's entry and is unchanged — the same card, the same ✕, the same long-press into a **Selection** — so what hangs below reads as that entry's contents rather than as six more entries. Every amount is a box the moment the fold is open, and tapping one opens the app's own ingredient picker; every act writes at once, because these lines are another view of facts the occasion already records rather than a second editing mode over them. What the fold corrects is **what is in** the dish; how much of the dish was eaten is still the correction sheet's question alone. It is open or shut for this sitting only, and nothing remembers which.
