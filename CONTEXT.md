...
**Twin alias**:
A name a Search index row also answers to: the description USDA published for the other record of the same merged identity, which the merge discarded when it kept the base record's name. `Spinach, mature` answers to `Spinach, raw`, `Millet, whole grain` to `Millet, raw`. It is **search-only** — the food is shown and staged under its own name, and unlike a Vocabulary map hit nothing is appended to what the card displays. It asserts retrievability, never identity: it says this row answers to that name, never that the two names are the same food. Carried in the row's `also`, and scored as a name in its own right, so a row is ranked as the best of all its names. See ADR-0050.
_Avoid_: Synonym, alias (bare — a Curated stand-in's `aliases` and the Vocabulary map are both different things), alternative name, former name
