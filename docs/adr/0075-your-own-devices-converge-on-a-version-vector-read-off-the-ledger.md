...
**Implemented:** #395 `3d32d18` `1a08f36` `15eb29a` — §6, §7, §8, §11 and §13's two carried refusals, live. The version vector is `src/lib/db/version-vector.ts`: one module owning the query that computes one, the `WHERE` that filters by one and the shape one takes on a wire, so the three cannot disagree about what _above_ means. It is read by `readLedgerVersionVector` and narrows `readLedgerPage`, which is now the same walk for a jar-wide export, a Facet-scoped one and a sync delta. `src/lib/p2p/first-sync.ts` runs the act: vectors out, chunks both ways, a closing vector, and the Paired Device record written on that one line. §7's per-chunk seal is `sealed-frame.ts`'s new label, carrying the lane as well as the sequence — one room key runs both directions, so without the lane a frame reflected at its own sender would verify. §8's advance-on-receive needed no new machinery, as it said. Three things the record does not say, found in building it: an **empty chunk is the final one**, which is the whole end-of-stream marker because `readLedgerPage` returns nothing only when the walk is finished; the closing vector must be read **after** the peer's last row has landed, or it understates what this device holds and the peer's first deposit re-sends what this session just delivered; and §11's progress is **counts rather than a bar**, because neither side knows the total until the walk ends. §1, §2, §4's Devices screen and §5's room half are ADR-0096's and are not built here; §9's `NotesStore` defect, §10's history surface and §12's one-sided state are unbuilt  
**Amended by:** [ADR-0103](0103-a-pairing-is-scoped-to-the-facet-its-act-ran-in.md) (§6's version vector is re-keyed by originating device **and** Tracked Domain; it stays a read of `datoms`, and its argument against a scalar watermark survives widened to a second axis)
