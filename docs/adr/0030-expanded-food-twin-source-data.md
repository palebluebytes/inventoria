...
- **Food-bearing twins now carry** `food/name`, `food/category`,
  ...
  `docs/eavt-vocabulary.md`. (The clause requiring the food twin shape in
  `V1_REQUIREMENTS.md` Module A to be updated is discharged: that document is
  archived at `docs/history/V1_REQUIREMENTS.md` and `docs/eavt-vocabulary.md` is now
  the sole registry.)
