...
**Implemented:** #292 `87f5dcd`, `0b201ac`, `b27d88c` — §2, §6, §8 and §9 as `src/lib/p2p/safari-tab.ts`, `takeCodeHandover` in `src/lib/p2p/receive-link.ts`, `src/lib/views/food/CodeHandover.svelte` and the boot-order gate above `dbClient.init` in `src/App.svelte`; §4, §12 and §13 as `src/lib/p2p/pasted-link.ts` and `src/lib/views/food/MealLinkField.svelte`, mounted on the Scan way in by `src/lib/views/food/FoodStager.svelte`; §11.8's replaced reason in `src/lib/p2p/send-words.ts`. §3 needs no code: the way out never grew a platform branch. §7 stays empirically unrun, and the device half of §6 is [#287](https://github.com/palebluebytes/inventoria/issues/287)
