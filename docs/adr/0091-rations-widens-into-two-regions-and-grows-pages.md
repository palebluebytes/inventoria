...
**Implemented:** §1 by #347 — the Amendment on ADR-0089 itself, since what §1 changes is another record rather than any code. §2, §3 and §4 by #342 — `app.css` (the one shared `.main`, centred and capped, and the three measures `--measure-solo`, `--measure` and `--rail`), `App.svelte` and `Rations.svelte` (each losing its copy of a rule that was duplicated character for character), `views/food/DailyDashboard.svelte` (the `.day` element the two-region grid is written on, scoped to `.rations` so the root's Food tab does not grow a rail, and the room under the last meal moved onto the timeline), `tests/unit/shell.test.ts` (§4 is a decided absence, so the unpinned rail is asserted rather than drawn). The rail's own top block by #344 — `views/food/MonthCalendar.svelte`, `lib/food/logged-days.ts` (the pure day-key fold and the local-fields conversion the calendar reads through), `views/food/DailyDashboard.svelte` (which swaps the week strip and the `.dashboard-header` banner out for it at this width, the first exercise of §1), `tests/unit/logged-days.test.ts` and `tests/unit/month-calendar.test.ts`; the shipped weight is +58,485 B on Rations (+57.1 KiB, +0.58%) and +8,502 B on the root (+0.09%), measured build to build — the Consequences below say "about 56 KiB" and "~7.5 KiB", which were the prototype's figures and are what these replace. `facets/registry.ts`'s `precacheBytes` moves further than either, because it had not been re-declared since #343 and takes up that drift too, and it says so. §5's `inline` surface by #341 — `ui/BottomSheet.svelte`, `views/food/FoodSettingsSheet.svelte`, `views/food/RecipeLibrarySheet.svelte` — and the `Rations.svelte` comment §5 makes false by #347. §5's pages themselves by #345 — `food/pages.ts` (the roster in the header's order, what each page's one control is called and what a legend says it does, and the width a page exists at — which is where the shell half of that lives, so a shell with no pages never reads a width and a resize cannot reach a screen that has only ever shown sheets), `ui/breakpoints.ts`'s `watchAtLeast` (the only width this app reads from JavaScript, because a page is a component mounted rather than a box laid out), `views/FoodView.svelte` (one opening instead of two booleans, the title as the way back, the header's icons as navigation with the current one inverted, and one call site per surface switched by `inline`), `Rations.svelte` (`hasPages`, the shell saying what it can hold, which the root does not pass) and `tests/unit/pages.test.ts`. Two of the three pages, and the third by #346. §8's roster and gate by #343 — `lib/ui/breakpoints.ts`, `tests/unit/breakpoints.test.ts` — and its seven deletions by #342 — `views/FoodView.svelte` ×1, `views/food/DailyDashboard.svelte` ×5, `views/food/CommitButton.svelte` ×1. An eighth of the same shape survives in `app.css`'s `.main` gutter, because #342 enumerated the seven it was deleting and did not name it; both that file and `shell.test.ts` say so rather than calling it a shape rule. §6 and §7 by #346 — `lib/food/reports.ts` (the period and the three folds, all pure and all taking their clock as an argument), `views/food/ReportsPage.svelte`, `lib/food/pages.ts` (the roster's third member, and `hasSheetForm`/`pagesShownAt`, which are where §7 lives: the fact that Reports has no second shape belongs to the page, and the width-filtered roster is what both the header and the legend loop so neither can describe a mark the other does not draw), `lib/food/nutrient-display.ts` (`nutrientLabel`, the full-word sibling of `nutrientShortLabel`, so "Carbs" is not written out a second time beside the catalogue entry that already says it), `views/FoodView.svelte` and `tests/unit/reports.test.ts`, `tests/unit/reports-page.test.ts`, `tests/unit/pages.test.ts`. Six things §6 left open and this settled. **A rolling week is the seven days ending today**, and a month and a year are counted by the calendar rather than as 30 or 365 days: the window opens the day after the step back, and the step back **clamps** to the last day of the month it lands in rather than letting `Date` normalise it. Normalising is what a first draft does and it is wrong — `new Date(2026, 1, 31)` neither throws nor clamps, it overflows to 3 March, so a month stepped back off 31 March lands three days _after_ February and the window is 28 days long with no February in it at all. **A period is half-open in milliseconds**, opening at a local midnight and closing at the local midnight of the day after the one it covers, rather than at `getDayBounds`' inclusive `23:59:59.999`: a period is a run of whole days and an exclusive end says so with no fudge factor to get wrong. **Each macro's share is taken against the three macros' own Atwater sum, never against the logged calories** — the two disagree, because alcohol, polyols and the fibre gap (#122) live in the difference, and dividing by the second leaves a remainder nothing on the reading accounts for. **A day with food but no frozen energy keeps its row with its reading absent**, which `Meter` already draws as a striped role-less track rather than a bar of no length — ADR-0048 reaching the pixel. **The ranking of foods is returned whole**, with no top-N, because a report that quietly cut its tail would read as the whole period while describing part of it. And **a far-off day carries its year**, because a yearly period covers two Augusts and `dayLabel`'s "Mon 11 Aug" would be the same four characters for both. The Consequences' one unproved claim — that Rations' own shell had never been photographed — by #348: `tests/visual-catalog.spec.ts` (a third catalogue at `/food/`, the day in both projects and the three pages above the shell breakpoint, reading readiness off the day's skeleton rows because the `.db-badge` it used to wait on is in a sidebar this Facet does not have), `tests/layout-invariants.spec.ts` (the sweep extended to Rations and to a 1920×1080 viewport, with the surfaces to sweep read off `lib/food/pages.ts` so a fourth page is swept without anyone remembering to add it) `tests/support/rations.ts` (the goto and the readiness wait both specs needed, which is where the argument for reading readiness off the day is written once — including the positive marker taken before the absence, since "no skeleton rows" is also true of a document Svelte never mounted into) and `tests/unit/shell.test.ts` (the box those captures flatten: one scroll box, and all four safe areas where the root reserves three). The `chromium` project stays at 1280×720, which is what bounds §8's breakpoint. `precacheBytes` needed no move there — the build measured 9,416,531 B and 10,177,101 B, which is what #346 had declared to the byte.
