...
**Status:** Accepted  
**Date:** 2026-09-01  
**Implemented:** §2's Local Logs rows, §5, and §7's surface by [#310](https://github.com/palebluebytes/inventoria/issues/310) — `src/lib/views/food/FoodSettingsSheet.svelte` is the named, full-height _Rations settings_ surface, titled off `src/lib/facets/registry.ts`; a log channel names the Tracked Domain that writes it in `src/lib/logs/log-facility.ts` and `channelsOfFacet` derives a Facet's list from the domains it already declares; `src/lib/stores/device-settings.ts` holds one export door per Facet, on the two keys ADR-0085 §4 named. §7's three sections are two until the "Your data" blocks arrive; §2's remaining rows and those blocks are [#303](https://github.com/palebluebytes/inventoria/issues/303) and [#311](https://github.com/palebluebytes/inventoria/issues/311).  
**Implemented:** §2's API Credentials rows, §4 and §6 by [#303](https://github.com/palebluebytes/inventoria/issues/303) — `src/lib/views/media/MediaSettingsSheet.svelte` is the affordance §4 commissioned, opened by a gear on `MediaView`'s header and carrying the TMDB key; the scraper proxy field is gone from `SettingsView` and `stores/device-settings.ts` kept the read and lost its setter, so `ingestion/fetcher.ts` no longer names a field that is not there; the API Credentials card dissolved with them. §6's readout is gone from `views/logs/LogSettingsSection.svelte`, and the bar it drew went with its last reader — `search-log.ts` records the channel and computes no verdict. §2's remaining row is the Facet-scoped wipe and export, which is [#311](https://github.com/palebluebytes/inventoria/issues/311).  
...
**Amended by:** [ADR-0103](0103-a-pairing-is-scoped-to-the-facet-its-act-ran-in.md) (§9's _a food-only user therefore has no p2p at all_ is now false in both halves, because a lane scoped to a Facet's own domains passes clause (b) where a jar-wide pairing failed it; §2's table gains a Paired devices row)
