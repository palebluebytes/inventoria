...
What has to be serialised is the read and the write together, which is the errand, and the
errand is the whole of `openAppWake`'s two syncs rather than a deposit inside one.
...
**Where the runtime has no `LockManager`, the errand runs unserialised**, guarded the way every
`localStorage` access in this app is guarded. A device with one open has nothing to serialise
against, and refusing to converge to avoid a race that needs a second open would trade a
