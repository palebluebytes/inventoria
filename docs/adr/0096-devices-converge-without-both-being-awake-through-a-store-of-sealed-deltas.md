...
**So §11's own words are withdrawn to that extent, by name.** Its step-3 paragraph reads _the
peer's next rewrite is refused by the etag precondition **and never recreated**_, and the second
half of that was written before this exit existed. The refusal stands and is load-bearing; the
recreate that answers it is the whole repair, and a reader who takes §11 at its word there builds
the frozen pairing back.
...
## Amendment (2026-09-13, #418): a wake's errand is one at a time at the origin

§3 prices convergence in wakes and §5 makes a deposit's rewrite conditional, and between them
they assume one wake at a time without saying so. Nothing enforced it. `openAppWake` runs on
every open, and a root browser tab beside the installed root is **two opens over one
`localStorage` jar and one ledger** — the cadence's queue reaches only as far as its own open,
and the conditional `PUT` does not reach across at all.

### The two write orders can disagree, and the record is the one that decides

The rewrite is conditional on the etag this device's own `PUT` returned, and this record's own
2026-09-12 Amendment answers a refused one with an **unconditional recreate**. Both wakes also
write `deposit_standing` into one shared record, last write wins. So:

1. Wake A `PUT`s its delta. The object is D_A.
2. Wake B's conditional rewrite is refused and recreates unconditionally. The object is D_B.
3. Wake A's record write lands last. The standing is `{ etag: E_A, brings: V_A }`.
4. The peer collects **D_B** and acknowledges the index.
5. This device merges `standing.brings` = **V_A** into `peer_vector`.

Any row V*A covers that D_B did not carry is now credited to a peer that never received it, and
`vectorAboveMatch` withholds it on every later sync. It is the same hazard §5's rider names from
the depositor's side — \_crediting the fuller rewrite would skip the rows only it carried,
permanently* — reached here from the record's side instead.

**It is latent rather than loud.** Two wakes on one device normally compute the same delta from
the same ledger against the same `peer_vector`, so whichever object survives matches whichever
standing survives. They differ only where the ledger changed between the two reads, which is a
meal logged in one window while the other is mid-wake.

> **Collecting, acknowledging and depositing are one errand, and two errands may not overlap at
> one origin. A wake takes an origin-scoped lock around the whole of one; where the runtime
> offers no lock the errand runs anyway.**

### The lock goes round the errand, and not round the write

A lock on the record write alone would close nothing. Both errands read `deposit_standing`
before either writes, so the disagreement is already settled by the time the first `PUT` leaves.
What has to be serialised is the read and the write together, which is the errand. **Each of
`openAppWake`'s two syncs is one errand and takes the lock on its own**, rather than one lock
spanning both or one guarding a deposit inside either: a wake is open for as long as the app is,
and a lock held for that long would be a second tab that never converges.

`navigator.locks` is the mechanism because it is scoped to the **origin**, which is exactly the
set of wakes that share a jar: two tabs, a tab beside an install, two installs, and — once
ADR-0103 §9 lands — a root tab beside an installed Rations. A `localStorage` flag is not atomic,
and a `BroadcastChannel` election needs a timeout to decide that nobody answered.

**The loser waits rather than skipping.** A wake that declined to run would be an open that did
not collect, and the promise is stated in opens; a wake that waits re-reads the record
afterwards, finds the lane already advanced, and has nothing left to do. Waiting costs latency
on a path with no deadline. Skipping costs the open the user made.

**Where the lock cannot be had, the errand runs unserialised.** A device with one open has
nothing to serialise against, and refusing to converge to avoid a race that needs a second open
would trade a certainty for a possibility. There are **three** ways not to have it and the
fallback covers all three, which is more than a presence check reaches: the API is absent under
the Node unit runner and in any insecure context; a browser that has taken it away can throw on
the accessor rather than answer `undefined`, which is the `try` every `localStorage` access in
this app already carries; and an opaque origin **refuses the request**, rejecting before the
callback ever runs. The third is told from an errand that failed under a lock it was granted by
whether the callback ran, because they want opposite answers — an errand that already ran may
not be run again, or the device deposits twice.

### What this does not claim

It is one origin. Two devices depositing into one lane is not this — that is two ends of a
pairing and §5's conditional rewrite is what governs it. And a lock held by a tab that hangs
mid-errand holds the other tab's errand with it until the browser reclaims it, which is a
deferred wake rather than a lost one.

**And the harm above is stated in one direction more than it is reachable in.** Two overlapping
_deposits_ cannot produce the overstating case on their own: both compute their delta against
the same `peer_vector` over a ledger that only grows, so the later one's is the superset and the
standing that survives can only **understate** what the object carries — which costs a re-sent
row an import ignores. The overstating case needs the two errands to disagree about
`peer_vector`, which is a collection running beside a deposit, or a ledger that shrank under
them both. The lock covers all of it because it serialises the errand rather than the case, and
the measurement is `tests/unit/wake-lock.test.ts`'s: the disagreement it drives is real in both
the etag and `brings`, and it is the understating direction.
