...
**Amended by:** [ADR-0081](0081-a-meals-closure-is-bounded-by-kind-not-by-reachability-alone.md) (§8.4 and §8.5 are enforced by entity kind, not by reach alone; §8 gains an eighth refusal on attribute namespace; and §8's premise that an unknown attribute rides unread is corrected, since two projections scope by attribute); [ADR-0110](0110-an-impromptu-recipe-is-identified-by-its-ingredients-and-named-only-if-you-keep-it.md) (§5's minted twin ids can now collide by design — a content-derived `recipe:` id is how an impromptu recipe is identified — so "cannot collide" no longer holds for them)
