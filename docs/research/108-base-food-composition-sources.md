...
| **Fibre**     | **93%**                             | **56% (51% under `1079`)**         | **97%**                           | **51% AOAC / 84% NSP**           |
...
**Which ids count is the choice that matters,** which is why Foundation's fibre cell carries both figures. 185 records report fibre under `1079` (total dietary) and 17 more report it only under `2033` (AOAC 2011.25); the app fills the panel from either, so the union is the honest answer to "what can we show", but the two are different assays and §7 is right that adding one basis to another silently is how unit errors happen. The same choice separates 88% energy from 26%, Foundation reporting energy as Atwater factors and mostly omitting `1008`. The non-null `amount` rule, by contrast, changes nothing: no entry under a panel id lacks a numeric amount in either archive. The CIQUAL and CoFID columns were measured on each table's own conventions, so read any cross-table comparison of fibre as approximate in the first decimal, not to the point.

**Where else these numbers appear.** Three restatements outside §4 move with it, and are left in place rather than edited: §1's recommendation reads SR Legacy's fibre as **92%** (now 93%), calcium and sodium as 98% (now 99% each) and saturated fat as 95% (now 96%), with energy 100% and vitamin C 94% unchanged; §3.1's Foundation row reads "thin: fibre 54%, vitamin A 12%, B12 18%" (now 56%, 14%, 18%); and [ADR-0045](../adr/0045-usda-stays-the-base-food-composition-authority.md)'s Context reads folate as 87% against CIQUAL's 43% (now 88%). Every one of them is a single point except Foundation's, and none reverses a comparison.
