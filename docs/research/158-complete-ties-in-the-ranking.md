...
**Discharged (2026-08-24, [#162](https://github.com/palebluebytes/inventoria/issues/162)).** `beef` was cut as its own ticket and answered: an eighth name key, `wholeness`, reading USDA's `composite of` marker and a closed list of whole qualifiers naming a separated fat. `beef` now leads with `Beef, composite of trimmed retail cuts, …, all grades, raw` and its tie is 12 rows rather than 412; `veal` leads with its composite and ties 2. §4's finding is what made the key legible — it fires on `pork` and `lamb` and confirms the leads they were dealt, taking those ties to 5 and 8 — and it also overturns §4's own conclusion that a candidate must be shown to tie uniformly across the four first. The figures in §2 and §3 above stand as measured on the day; re-run `pnpm usda:ranking-ties` rather than quoting them. The 200+ kcal band is now 130 queries.
