...
// ── ADR-0048 §6: the unloggable record, asked in ONE place ──────────────────
// `fdcReportsNoEnergy` is asked at generation time (`scripts/usda-bundle.mjs`)
// and again at log time by the food card. What this locks is that it is asked
// once: it is written in terms of the app's own `buildNutritionPanel` and
// `reportsNoEnergy`, so an id joining `ENERGY_IDS` moves both answers at once
// and neither can drift from the other. That is also why it is the one ADR-0048
// filter that did not move to `usda-food-kind.test.ts` (#146).
