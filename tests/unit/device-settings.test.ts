...
// stubbing the global it will read. The store itself is the shared adapter's
// (#222), the same one the secrets suite uses.
const loadPrefs = () => import("../../src/lib/stores/device-settings");
...
describe("device settings (ADR-0085: a setting is never a datom)", () => {
  it("reads its values at import, with no ledger and no await", async () => {
    ...
    const prefs = await freshModule(loadPrefs);
  ...
  describe("the nutrition panel's fold", () => {
    it("defaults to open, and only an explicit false shuts it", async () => {
      const [prefs, ls] = await freshModuleWithStorage(loadPrefs);
      expect(get(prefs.nutritionPanelOpen)).toBe(true);
  ...
  describe("the visible-nutrient selection", () => {
    it("defaults to Protein/Fat/Carbs/Fibre when unset", async () => {
      const [prefs] = await freshModuleWithStorage(loadPrefs);
      expect(get(prefs.visibleNutrients)).toEqual(DEFAULT_VISIBLE_NUTRIENTS);
    });
    ...
    it("falls back to the default when the stored value is malformed", async () => {
      ...
      expect(get((await freshModule(loadPrefs)).visibleNutrients)).toEqual(
  ...
  describe("the scraper proxy URL", () => {
    it("reads the stored value, and an env fallback when unset", async () => {
      ...
      expect(get((await freshModule(loadPrefs)).scraperProxyUrl)).toBe(
        "https://env-proxy/?url="
      );
    ...
    it("has no setter, because the field that wrote it was deleted", async () => {
      ...
      // corrected.
      stubLocalStorage();
      expect(await freshModule(loadPrefs)).not.toHaveProperty(
        "setScraperProxyUrl"
      );
    });
  });
  ...
  describe("the food targets (ADR-0085 §5: moved out of the ledger)", () => {
    ...
    it("filters targets to reach-toward keys and finite numbers", async () => {
      ...
      expect(get((await freshModule(loadPrefs)).foodTargets)).toEqual({
        protein: 160,
      });
    });
    ...
    it("filters limits to limit keys and finite numbers", async () => {
      ...
      expect(get((await freshModule(loadPrefs)).foodLimits)).toEqual({
        sodium_content: 1.5,
      });
    });
    ...
    it("filters the calculated set to the personalizable keys", async () => {
      ...
      expect(get((await freshModule(loadPrefs)).foodCalculatedTargets)).toEqual(
        {
          energy: 2200,
          protein: 130,
          fiber_content: 40,
    ...
    it("reads a profile back all-or-nothing, never half-seeded", async () => {
      ...
      expect(get((await freshModule(loadPrefs)).foodProfile)).toBeNull();
    });
